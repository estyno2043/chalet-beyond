...
export function HeroSCV() {
  ...
  useEffect(() => {
    ...
    // The loop used to start here and never stop. The component does not
    // unmount when the hero scrolls away, so it kept reading scrollY and
    // touching currentTime while the guest was down at the booking form —
    // competing for frames with whatever they were actually looking at.
    // It now runs only while the hero is on screen.
    const start = () => {
    ...
    // Start first, then let the observer pause it. The other way round — only
    // starting once the observer reports the hero visible — makes the whole
    // hero depend on a callback arriving. If it ever does not, nothing moves at
    // all. This way the observer can only ever take work away, never be the
    ...
    // A backgrounded tab throttles rAF to a crawl anyway; stopping outright
    // means it does not resume mid-seek when the guest comes back.
  ...
  return (
    <div
      id="hero"
      ref={rootRef}
      style={{ height: totalHeight, background: "oklch(0.06 0.008 55)" }}
    >
